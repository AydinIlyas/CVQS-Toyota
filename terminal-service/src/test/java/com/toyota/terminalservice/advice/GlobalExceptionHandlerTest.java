...
class GlobalExceptionHandlerTest {
    ...
    @Test
    void handleTerminalNotFoundException() {
        //given
        String message="Terminal not found";
        TerminalNotFoundException terminalNotFoundException=new TerminalNotFoundException(message);

        //when
        String path="/test";
        HttpServletRequest request= mock(HttpServletRequest.class);
        Mockito.when(request.getRequestURI()).thenReturn(path);
        ResponseEntity<Object> response=globalExceptionHandler.handleTerminalNotFoundException
                (terminalNotFoundException,request);
        //then
    ...
    @Test
    void handleTerminalAlreadyExistsException() {
        //given
        String message="Terminal already exists";
        TerminalAlreadyExistsException terminalAlreadyExistsException=new TerminalAlreadyExistsException(message);

        //when
        String path="/test";
        HttpServletRequest request= mock(HttpServletRequest.class);
        Mockito.when(request.getRequestURI()).thenReturn(path);
        ResponseEntity<Object> response=globalExceptionHandler.handleTerminalAlreadyExistsException
                (terminalAlreadyExistsException,request);
        //then
        ErrorResponse errorResponse=(ErrorResponse) response.getBody();
        Assertions.assertNotNull(errorResponse);
